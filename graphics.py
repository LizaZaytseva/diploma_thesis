...
class Graphics(object):
    ...
    def plot_for_locations(self, loc_list):
        ...
        for i in range(0, 7):
            for loc in loc_list:
                ...
                if loc_index == 0:
                    ax = df_location.plot(x='Год', y=self.param[i], color=self.color[loc_index % 7],
                                          marker=self.marker[7 - loc_index % 8],
                                          linestyle=self.line[loc_index % 3], title=self.param[i], label=loc,
                                          figsize=(30, 10))
                else:
                    df_location.plot(ax=ax, x='Год', y=self.param[i], color=self.color[loc_index % 7],
                                     marker=self.marker[7 - loc_index % 8],
                                     linestyle=self.line[loc_index % 3], title=self.param[i], label=loc)
            plt.hlines(y=0, xmin=2008, xmax=2023, linewidth=4)
            leg = ax.legend(loc='upper right')
            plt.tight_layout()
            plt.savefig(f'Results/Descr_statistics/all_loc_{self.param[i]}_stat.png')
    ...
    def plot_for_years(self):
        ...
        for i in range(0, 7):
            ax = df.plot(x='Год и месяц', y=self.param[i], color=self.color[i], title=f'{self.param[i]}', marker='o',
                         figsize=(20, 4))
            ax.xaxis.set_major_locator(mdates.MonthLocator(interval=3))
            ax.xaxis.set_major_formatter(mdates.DateFormatter('%m/%Y'))
            plt.tight_layout()
            plt.savefig(f'Results/Descr_statistics/all_years_stat_{self.param[i]}.png')

    # Графики для ср. значения и медианы по типу леса
    def plot_forest_types(self):
        ...
        for p in range(0, 7):
            for i in range(0, 3):
                ...
                if i == 0:
                    ax = df_t.plot(x='Год и месяц', y=self.param[p], color=self.color[i], label=labels[i],
                                   title=self.param[p], marker='x', figsize=(28, 4))
                    ax.xaxis.set_major_locator(mdates.MonthLocator(interval=2))
                    ax.xaxis.set_major_formatter(mdates.DateFormatter('%m/%Y'))
                    ax.set_ylabel('Кол-во клещей на 1 флагочас')
                else:
                    df_t.plot(ax=ax, x='Год и месяц', y=self.param[p], color=self.color[i], label=labels[i], marker='x')
            plt.tight_layout()
            plt.savefig(f'Results/Descr_statistics/forest_{self.param[p]}.png')
    ...
    # Графики по локациям для описательной статистики
    def plot_for_location(self, file, location):
        ...
        fig.tight_layout()
        for i in range(0, 7):
            df_location.plot(ax=axes[i], x='Год', y=self.param[i], color='b', title=f'{self.param[i]}', legend=False,
                             xlabel='', marker='x')
    ...
    def plot_for_year(self, year):
        ...
        for i in range(0, 7):
            df.plot(ax=axes[i], x='Месяц', y=self.param[i], color='b', title=f'{self.param[i]}', marker='o', legend=False)
        axes[0].set_ylabel('Кол-во клещей на 1 флагочас')
        fig.tight_layout()
        fig.savefig(f'Results/Descr_statistics/stat_{year}.png')

    # Графики для описательной статистики по месяцам
    def plot_for_months(self):
        ...
        df.drop(labels=[0], axis=0, inplace=True)
        for i in range(0, 7):
            df.plot(x='Месяц', y=self.param[i], color=self.color[i], title=f'{self.param[i]}', marker='o', legend=False,
                    ylabel='Кол-во клещей на 1 флагочас')
            plt.tight_layout()
            plt.savefig(f'Results/Descr_statistics/stat_months_{self.param[i]}.png')

    # Графики для описательной статистики по месяцам
    def plot_for_types(self):
        ...
        for i in range(0, 7):
            ax = df_r.plot(x='Год', y=self.param[i], color=self.color[1], label='I.Ricinus', title=self.param[i],
                           marker='x', ylabel='Кол-во клещей на 1 флагочас', figsize=(10, 5))
            df_p.plot(ax=ax, x='Год', y=self.param[i], color=self.color[2], label='I.Persulcatus', title=self.param[i],
                      marker='o')
            plt.tight_layout()
            plt.savefig(f'Results/Descr_statistics/stat_types_{self.param[i]}.png')

    # Линейная регрессия
    def plot_regression(self, X, y, arr, res_reg, location):
        plt.clf()
        plt.scatter(X, y)
