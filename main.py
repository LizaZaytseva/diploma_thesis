...
def correlation(plots, data_analysis, df_ric, df_pers):
    ...
    types = ['Имаго I.Ricinus', 'Нимфы I.Ricinus', 'Имаго I.Persulcatus', 'Нимфы I.Persulcatus', 'Количество']
    ...
    df_all_types = pd.merge(data_analysis.df, df_corr, how='left', on=['Год', 'Месяц'])
    ...
    for t in range(0, 5):
        for f in range(0, len(factors)):
            if t < 2:
                ...
                res = data_analysis.corr_estimation(df_all_types, types[t], factors[f], res)
                plots.plot_corr(df_all_types, res[len(res) - 1]['factor 1'], res[len(res) - 1]['factor 2'],
                                res[len(res) - 1]['corr. coef'], res[len(res) - 1]['coef. t'])
            else:
...
def main():
    ...
    data_analysis.forest_type('Results/Descr_statistics/forest_types2.xlsx')
    # Графики
    ...
    data_analysis.reg_analysis(True, 'Results/Descr_statistics/res_location.xlsx', 'Results/Regression_results.xlsx', plots)
